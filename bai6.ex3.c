#include <stdio.h>
#include <stdlib.h>

/* run this program using the console pauser or add your own getch, system("pause") or input loop */

void main()
{

printf("The number 555.55 in various forms: \n");

printf("In float form without modifiers:\n");
printf("%f]\n",555.55);
printf("In exponential form without any modifier:\n");
printf("%e]\n", 555.55);
printf("In float form with modifier:\n");
printf("%-f]\n", 555.55);
printf("In float form with digit string 10.3 as modifier\n");
printf("[%10.3f]\n", 555.55);
printf("Infloat form with modifier:\n");

printf("[%.0f]\n",555.55);


printf("In float form with and digit string 10.3");

printf("as modifiers:An");

printf("%-010.3f]\n", 555.55);

printf("In float form with - O");

printf("and digit string 10.3 modifiers:\n");

printf("%-010.3f]\n", 555.55);

printf("In exponential form with 0");

printf(" and digit string 10.3 modifiers:\n");

pintf("[%010.3f\n", 555.55);

printf("In exponential form with - 0");

printf(" and digit string 10.3 as modifiers: An");

printf("%-010.3f]\n\n",555.55);
}
